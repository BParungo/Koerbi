...
function onPointerMove(event: PointerEvent) {
  ...
  // Activate swipe only when horizontal movement dominates
  if (!swiping.value) {
...
<template>
  <div class="relative overflow-hidden rounded-lg" :class="{ 'opacity-50': item.done }">
    <!-- Red background with trash icon (revealed by swipe) -->
    <div
      v-if="swipeOffset < 0"
      class="absolute inset-0 flex items-center justify-end bg-destructive px-4"
      aria-hidden="true"
    >
    ...
    <!-- Swipeable item -->
    <div
      class="relative flex items-center gap-3 rounded-lg bg-background px-2 py-2 transition-colors hover:bg-muted/50"
